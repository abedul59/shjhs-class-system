<template>
  <div class="control-panel">
    ...
    <div v-if="examData.isExamModeEnabled && isIpBrownlisted" class="exam-trigger-container">
      <button @click="$emit('enterExam')" class="enter-exam-btn">切換至大考看板模式 🎯</button>
    </div>
  </div>
</template>
...
const props = defineProps({
  ...
  clockFontSize: Number, currentTime: String, unreadMsgCount: Number, scheduleDisplay: Object,
  scheduleButtonConfig: Object, isIpBrownlisted: Boolean, examData: Object,
  indexButtonSettings: { type: Object, default: () => ({}) },
  isScheduleButtonVisible: Boolean, seatingChart: Object, showSeatingChartLocal: Boolean,
  hygieneData: Object, showHygieneLocal: Boolean, isHistoryVisibleOnIndex: Boolean
})
...
@media (max-width: 768px) { .schedule-status-banner { flex-direction: column; gap: 10px; text-align: center; } }
</style>
