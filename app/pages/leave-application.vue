...
const previewSubject = computed(() => {
  const demoName = students.value.length > 0 ? `${students.value[0].seat_number}號 ${students.value[0].real_name}` : '1號 測試學生'
  return emailSubjectTpl.value.replace(/{student_name}/g, demoName)
})
const previewBody = computed(() => {
  const demoName = students.value.length > 0 ? `${students.value[0].seat_number}號 ${students.value[0].real_name}` : '1號 測試學生'
  return emailBodyTpl.value
    .replace(/{student_name}/g, demoName)
