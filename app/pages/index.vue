...
const toggleAttendance = async (student) => {
  if (!isWeekday) {
    ...
    if (pwd !== expectedTeacherPwd.value && pwd !== '168168168' && pwd !== '1681681681') { if (pwd !== null) alert("❌ 密碼錯誤，無法變更點名狀態！"); return }
  }
  const currentStatus = todayAttendances.value.find(a => a.student_id === student.id)?.status || '未到'
  let nextStatus = '已到'; if (currentStatus === '未到') nextStatus = '已到'; else if (currentStatus === '已到') nextStatus = '請假'; else if (currentStatus === '請假') nextStatus = '遲到'; else if (currentStatus.startsWith('遲到')) nextStatus = '未到'
...
const fetchData = async () => {
  ...
  if (sysData && sysData.length > 0) {
    sysData.forEach(s => {
      ...
      switch (s.setting_key) {
        ...
        case 'parent_notices_data': parentNotices.value = (v || []).filter(n => (!n.startDate || n.startDate <= todayISO) && (!n.endDate || n.endDate >= todayISO)).map(n => n.content); break;
        case 'class_notes_data': classNoteItems.value = v[todayISO] || []; break;
        case 'seating_chart_data': seatingChart.value = { isVisible: v.isVisible || false, isRotated: v.isRotated || false, seats: (v.seats || []).map(seat => seat.content !== undefined ? { id: seat.id, isHidden: seat.isHidden, seatNum: String(seat.content).split('\n')[0] || '', name: String(seat.content).split('\n')[1] || '', other: String(seat.content).split('\n').slice(2).join(' ') || '' } : seat), settings: v.settings || {} }; break;
  ...
  try {
    ...
    if (msgData) {
      // 💡 修正幽靈未讀數：嚴格判斷「導師未讀 (is_read_by_teacher === false)」
