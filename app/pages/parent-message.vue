  ...
  <div class="page-container">
    <div class="chat-wrapper">
      ...
      <div class="messages-container">
        ...
        <div v-else-if="messages.length === 0" class="system-msg">目前沒有訊息，您可以隨時留言給導師。</div>
        
        <div v-else class="message-list">
          <div 
            v-for="(msg, index) in messages" 
            :key="index" 
            :class="['message-bubble', msg.sender === 'parent' ? 'my-message' : 'teacher-message']"
          >
            <div class="message-info">
              <span class="sender-name">{{ msg.sender === 'parent' ? '家長' : '導師' }}</span>
      ...
      <div class="input-area">
        <input 
          ...
          :disabled="isLoading"
        />
        <button @click="sendMessage" class="btn btn-green send-btn" :disabled="isLoading || !newMessage.trim()">
          發送
        </button>
      </div>
    </div>
...
const checkSecurityAndLoad = async () => {
  ...
  try {
    ...
    if (blacklists && blacklists.length > 0) {
      ...
      // 即使是家長版，只要連到學校網段一樣強制攔截 (避免學生偷開)
      if (isBlocked) {
        alert('🚫 學校網域限制：校內網路禁止使用私訊功能。請使用個人手機網路再進行操作！')
        navigateTo('/') // 強制踢回首頁
        return 
      }
    }
...
const fetchMessages = async () => {
  // 模擬載入
  messages.value = [
    { sender: 'teacher', content: '家長您好，這裡是專屬私訊區，若學生在校有任何狀況，我會在此留言給您。', time: '08:00' }
  ]
}
