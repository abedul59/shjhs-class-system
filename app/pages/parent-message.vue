    ...
    <div class="message-card">
      <div v-if="!isVerified" class="verify-section">
        ...
        <form @submit.prevent="verifyIdentity" class="message-form">
          ...
          <div style="text-align: center; margin-top: 15px;"><NuxtLink to="/" class="back-link">返回打卡首頁</NuxtLink></div>
        </form>
      </div>

      <div v-else class="chat-section">
        <div class="chat-header"><h3>💬 與導師的私訊 ({{ verifiedStudentName }})</h3><button @click="logout" class="logout-btn">登出</button></div>
        <div class="chat-history" id="chatContainer">
          ...
          <div v-for="msg in chatMessages" :key="msg.id" :class="['chat-bubble', msg.sender_role === '家長' ? 'my-msg' : 'teacher-msg']">
            <div class="msg-info"><span class="sender">{{ msg.sender_role === '家長' ? '我 (家長)' : '👨‍🏫 導師' }}</span><span class="time">{{ formatTime(msg.created_at) }}</span></div>
...
const loadChatHistory = async () => { const { data } = await supabase.from('private_messages').select('*').eq('student_id', selectedStudentId.value).eq('chat_type', '家長').order('created_at', { ascending: true }); if (data) { chatMessages.value = data; scrollToBottom() } }
const sendMessage = async () => {
  if (!newMessage.value.trim()) return; isSending.value = true
  try {
    await supabase.from('private_messages').insert({ student_id: selectedStudentId.value, sender_role: '家長', chat_type: '家長', content: newMessage.value, is_read_by_teacher: false })
...
<style scoped>
/* 請保留您 parent-message.vue 原本的 CSS */
