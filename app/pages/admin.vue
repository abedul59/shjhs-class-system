      ...
      <main v-if="currentTab === 'homework'" class="data-table">
        ...
        <div class="homework-section">
          ...
          <div class="email-editor-section">
            ...
            <div class="form-group"><label>信件內容：</label><textarea v-model="hwEmailContentTemplate" rows="10" class="edit-input textarea-input"></textarea></div>
          </div>
...
const adminStudents = ref([]); const studentsList = ref([]); const studentsMap = ref({})
...
const adminNotices = ref([]); const boardLogs = ref([]); const isSavingBoard = ref(false); const isSendingEmail = ref(false)
...
const saveAdminNotices = async () => {
  ...
  if (!error) alert('✅ 須知儲存成功！前台已同步更新。')
  isSavingBoard.value = false
}
const sendNoticeEmail = async () => {
  alert('💡 此功能將依據設定發送全班家長信件 (待串接外部 API)')
}
...
const saveStudent = async (student) => {
  try {
    ...
    alert(`✅ ${student.real_name} 資料儲存成功！(家長資料欄位可透過關聯表進階更新)`)
  } catch(e) { alert('❌ 儲存失敗') }
}
const deleteStudent = async (id, name) => {
  if (confirm(`⚠️ 確定要刪除學生 ${name} 嗎？此操作無法復原！`)) {
